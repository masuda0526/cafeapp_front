...
const CafeGoodInput: React.FC = () => {
  const goodPoints: string[] = CafeFormStore(state => state.currentCafe.goodPoints);
  const [tmpPoints, setTmpPoint] = useState<Points[]>(goodPoints.map(p => { return { key: uuidv4(), value: p } }));
  const replacePoints = CafeFormStore(state => state.replaceData);
  const reflectStore = (points: Points[]) => {
    let l: string[] = [];
    points.forEach(p => l.push(p.value));
    replacePoints('goodPoints',l);
  }
